...
import VASL.build.module.map.ASLPredefinedSetup;
...
public class ASLRefreshPredefinedSetupsDialog extends JDialog {
    //private static final Logger logger = LoggerFactory.getLogger(VASSAL.configure.RefreshPredefinedSetupsDialog.class);
    ...
    private ASLPredefinedSetup apds;

    private final Set<String> options = new HashSet<>();

    public ASLRefreshPredefinedSetupsDialog(Frame owner) throws HeadlessException {
        super(owner);
    ...
    private void initComponents() {
        ...
        apds = new ASLPredefinedSetup();
        apds.setExtensionName("C:\\Users\\DougR\\Documents\\Programming Documents\\VASLBuilds\\extensions\\Scenario Setup Files.vmdx");
    }
    ...
    private void refreshScenarioSetups() throws IOException {
        ...
        final java.util.List<ModuleExtension> moduleExtensionList = mod.getComponentsOf(ModuleExtension.class);
        final boolean isRefreshOfExtension = !moduleExtensionList.isEmpty();

        final java.util.List<PredefinedSetup> modulePdsAndMenus = mod.getAllDescendantComponentsOf(PredefinedSetup.class);
        final java.util.List<PredefinedSetup> modulePds = new ArrayList<>();
            ...
            if (!warningPds.isEmpty()) {
                ...
                for (i = 0; i < warningPds.size(); i++)
                    log("|&nbsp; " + Resources.getString("Editor.RefreshPredefinedSetups.endWarnPds",
                            warningPds.get(i).getAttributeValueString(ASLPredefinedSetup.NAME),
     ...
     */
    private int promptConfirmCount(java.util.List<PredefinedSetup> pdsList) {
