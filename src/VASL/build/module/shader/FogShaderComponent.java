...
public class FogShaderComponent extends AbstractBuildable implements GameComponent, CommandEncoder {

    private static final Logger logger = LoggerFactory.getLogger(FogShaderComponent.class);

    private final char ENCODING_DELIM = '|';
    private final String COMMAND_PREFIX = "command.shader.fog" + ENCODING_DELIM;
    ...
    public Command getRestoreCommand() {
        ...
        if (env.isFog()) {
            ...
            restoreCommand = new ActivateFogShaderCommand();
        } else {
            logger.trace("restore: disabled");
            restoreCommand = new DeactivateFogShaderCommand();
        }
    ...
    public String encode(Command command) {
        ...
        if (command instanceof ActivateFogShaderCommand) {
            encoder.append(true);
