...
public class DustShaderComponent extends AbstractBuildable implements GameComponent, CommandEncoder {

    private static final Logger logger = LoggerFactory.getLogger(DustShaderComponent.class);

    private final char ENCODING_DELIM = '|';
    private final String COMMAND_PREFIX = "command.shader.dust" + ENCODING_DELIM;
    ...
    public Command getRestoreCommand() {
        ...
        if (env.isDust()) {
            ...
            restoreCommand = new ActivateDustShaderCommand();
        } else {
            logger.trace("restore: disabled");
            restoreCommand = new DeactivateDustShaderCommand();
        }
    ...
    public String encode(Command command) {
        ...
        if (command instanceof ActivateDustShaderCommand) {
            encoder.append(true);
