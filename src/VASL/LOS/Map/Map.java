...
public class Map  {
    ...
    /**
     * THIS CONSTRUCTOR IS USED BY LOSGUI ONLY; there is a separate constructor for use by VASL
     ...
     * @param passboardgridconfig the configuration of the base board before cropping and flipping
     ...
     */
     public Map(double hexWidth, double hexHeight, int width, int height, double A1CenterX, double A1CenterY, int imageWidth, int imageHeight, HashMap<String, Terrain> terrainNameMap, String passboardgridconfig, String passcropgridconfig, boolean isCropping){
        ...
        // set configuration to use (either passboardgridconfig or passcroprgridconfig) based on whether map/board is cropped or not
        String configtouse =  (isCropping ? passcropgridconfig : passboardgridconfig);
        this.A1CenterX = A1CenterX;
        if(configtouse.contains("Normal")) {  // no cropping
        ...
        else if(configtouse.contains("TopleftHalfHeight")) { // hex in A1 position is half height meaning cropped to odd-numbered row (B, D, F OR special board such as DaE)
            this.A1CenterX = (A1CenterX == BoardArchive.missingValue() ? BoardArchive.GEO_A1_Center.x : A1CenterX);
        }
        else if((configtouse.equals("FullHex") || configtouse.equals("FullHexOffset") || configtouse.equals("FullHexEqualRowCount")) && isCropping) {  // first col is full width and A1 position is full height
            this.A1CenterX = this.hexWidth / 2;
            this.A1CenterY = 32.25;
        }
        else if(configtouse.contains("FullHexHalfHeight")) {  // first col is full width and A1 position is half height
            this.A1CenterX=this.hexWidth/2;
        }
        else if(configtouse.contains("LeftHalf")) {   // left board edge is not cropped and so is half width
            this.A1CenterX=0;
        }
        else if(configtouse.contains("FullHexRightHalf")) {   // right board edge is not cropped and so is half width; left edge col is full width
            this.A1CenterX=this.hexWidth/2;
        }
        else if(configtouse.contains("FullHexEqualRowCount")) {
            // ToDo I think DaE is the only LOS board of this type - CHECK
            this.A1CenterX=A1CenterX;
        }

        if (configtouse.contains("HalfHeight") && isCropping) { // && !passcropgridconfig.contains("TopleftHalfHeightEqualRowCount")) { // hex in A1 position is half height meaning odd-numbered row (B, D, F, etc)
    ...
    /**
     * This method is called by the current GEO board constructor for VASL and by the LOSGUI constructor
     * it creates an empty hexGrid of the size required by the board
     */
    ...
    * it is used by the VASL constructors only NOT LOSGUI
    */
    ...
    //this method will only be called by the map constructor used by LOSDataEditor from within LOSGUI
    // if one of the non-standard, LOS-enabled HASL maps is in use (RBv3, RO, DaE, SG)
    private Hex [][] createtheHASLHexGrid(BoardArchive boardarchive) {
        // create the hex grid
        //ToDo check for cropping. For 671 these HASL maps are already kicked out if flipped
        //ToDo check need for and use of startcol, startrow
        int startcol =0; int startrow =0;
        hexGrid = new Hex[this.width][];
        if (boardarchive.getBoardName().contains("RBv3")){
            if (this.A1CenterY== this.hexHeight / 2) {
                for (int col = 0; col < (startcol + this.width); col++) {
                    hexGrid[col] = new Hex[this.height + (col % 2)]; // add 1 if odd
                    for (int row = 0; row < (startrow + this.height + (col % 2)); row++) {
        ...
        else if (boardarchive.getBoardName().contains("HT")) {
        ...
        else if (boardarchive.getBoardName().contains("DaE")){ // each col has equal number of rows
            if (this.A1CenterY== this.hexHeight / 2) {
                for (int col = 0; col < (startcol + this.width); col++) {
                    hexGrid[col] = new Hex[this.height];
                    for (int row = 0; row < (startrow + this.height); row++) {
        ...
        else if (boardarchive.getBoardName().contains("VotG")){ // each col has equal number of rows
            if (this.A1CenterY== this.hexHeight / 2) {
                for (int col = 0; col < (startcol + this.width); col++) {
                    hexGrid[col] = new Hex[this.height + (col % 2)]; // add 1 if odd
                    for (int row = 0; row < (startrow + this.height + (col % 2)); row++) {
        ...
        else if (boardarchive.getBoardName().contains("SaPF")){ // each col has equal number of rows
            if (this.A1CenterY== this.hexHeight / 2) {
                for (int col = 0; col < (startcol + this.width); col++) {
                    hexGrid[col] = new Hex[this.height + (col % 2)]; // add 1 if odd
                    for (int row = 0; row < (startrow + this.height + (col % 2)); row++) {
        ...
        else if (boardarchive.getBoardName().contains("SG")){
            if (this.A1CenterY == boardarchive.getA1CenterY()) {
                for (int col = 0; col < (startcol + this.width); col++) {
                    hexGrid[col] = new Hex[this.height + (col % 2)]; // add 1 if odd
                    for (int row = 0; row < (startrow + this.height + (col % 2)); row++) {
                        if (col >= startcol && row >= startrow) {
            ...
            else if (this.A1CenterY == boardarchive.getA1CenterY() - this.hexHeight /2 ){
                int evencol =0;
                for (int col = 0; col < this.width; col++) {
                    evencol = col % 2 == 0 ? 1 : 0;
                    hexGrid[col] = new Hex[this.height + evencol]; // add 1 if even
        ...
        else if (boardarchive.getBoardName().contains("BRT")){   // each col has equal number of rows
            if (this.A1CenterY== this.hexHeight / 2) {
                for (int col = 0; col < (startcol + this.width); col++) {
                    hexGrid[col] = new Hex[this.height];
                    for (int row = 0; row < (startrow + this.height); row++) {
    ...
    //this is the current constructor for all geo maps (not used by LOSGUI)
    public Map (LinkedList<VASLBoard> vaslboards, double passA1centerx, double passA1centery, HashMap<String, Terrain> terrainNameMap, int passwidthinhexes, int passheightinhexes, Rectangle mapBoundary){
        ...
        if (b.getName().equals("RBv3") || b.getName().equals("RO") || b.getName().equals("DaE") ||
                 b.getName().equals("SG") || b.getName().equals("VotG") || b.getName().equals("FB")) { //b.getName().equals("Dinant") ||
    ...
    //constructor for HASL and non-standard maps (used by VASL only)
    ...
    //new constructor for HASL and non-standard maps called ONLY by LOSDataEditor in LOSGUI
    public Map (BoardArchive boardarchive, double passA1centerx, double passA1centery, HashMap<String, Terrain> terrainNameMap, int passwidthinhexes, int passheightinhexes, Rectangle mapBoundary){

        this.width = passwidthinhexes;
        this.height = passheightinhexes;

        //Set the hex geometry
        this.hexHeight= boardarchive.getHexHeight();
        this.hexWidth=boardarchive.getHexWidth();
        this.A1CenterX = passA1centerx;
        this.A1CenterY = passA1centery;
        // initialize
        setTerrain(terrainNameMap);
        ...
        //ToDo handle all crop options - see if we can remove
        this.cropconfiguration = this.A1CenterX == 0 ? "HalfHexWidth" : "FullHexWidth";
        this.cropconfiguration += (this.A1CenterY == 0) || (this.A1CenterY == boardarchive.getA1CenterY() - (hexHeight /2)) ? "LeftHexHalfHeight" : "LeftHexFullHeight";
    ...
    public String [] getAllHexesInRadiusOf(Hex originHex, double arrayradius) {
        int maxhexes = (int) ((arrayradius / 2) * (1 + arrayradius) * 6) + 1;
        Hex startHex;
        int x = 0;
        String[] hexes = new String[maxhexes];
        hexes[x] = originHex.getName();
        // recursive loop
        for (String hexname : hexes) {
            startHex = getHex(hexname);
    ...
    private class LOSStatus {
        ...
        private LOSStatus(Location source, boolean useAuxSourceLOSPoint, Location target, boolean useAuxTargetLOSPoint, LOSResult result, VASLGameInterface vaslgameinterface) {
            ...
            if(source.getTerrain().isRooftop() && source.getLevelInHex() !=1) {
                sourceadj=-1;
            }
            if(target.getTerrain().isRooftop() && target.getLevelInHex() !=1) {
                targetadj=-1;
            }
                    ...
                    source.isDepressionTerrain() &&
                            (
                                (
                                    ...
                                    (targetElevation+ targetadj) - (sourceElevation+sourceadj) <= range && range!=1
                                ) ||   //evel diff = range due to depression cliffs (except when adjacent)
                                    (
                                        (LOSis60Degree || LOSisHorizontal) && exitsDepressionTerrainHexside()
                                    ) ||       //LOS along hexside; one exit hexside is a depression
        ...
        public boolean exitHexsideIsCrest() {
            if((sourceExitHexsides[0] != NONE && sourceHex.getBaseLevelofHex() !=  getAdjacentHex(sourceHex, sourceExitHexsides[0]).getBaseLevelofHex()) ||
            ...
            else if((sourceExitHexsides[1] != NONE && sourceHex.getBaseLevelofHex() !=  getAdjacentHex(sourceHex, sourceExitHexsides[1]).getBaseLevelofHex()) ||
        ...
        public boolean enterHexsideIsCrest() {
            ...
            else if((targetEnterHexsides[1] != NONE && targetHex.getBaseLevelofHex() !=  getAdjacentHex(targetHex, targetEnterHexsides[1]).getBaseLevelofHex()) ||
        ...
        /**
         * Determines the total elevation of the source and target locations used in a LOS Check
         * The base formula is Base Level of Hex (-2 to + 7 or 8) + Level of Location within the hex (as measured as the difference from the Base Level of Hex)
         * EG 2nd level of a building on a Level 1 Hill: total elevation = 1 (Base Level of Hex) + 2(Level of Location within Hex) = 3
         *
         * Special Cases where this formula does not work are handled
         *
         * ToDo fix inconsistencies in LOS code as of Jan 2025
         * There are known situations where the LOS code is not generating the required data correctly
         * In particluar, a number of special locations are being created using their total elevation instead of their level within the hex:
         * Bridges
         * Need to check Entrenchments
         * crest
         * Virtual Levels
         * building levels
         */
        private void setSourceAndTargetElevations() {
            ...
            sourceElevation = sourceHex.getBaseLevelofHex() + source.getLevelInHex() + leveladj;

            leveladj = 0;
            if (!target.isCenterLocation() && targetHex.isDepressionTerrain()){
                leveladj=1;
            }
            targetElevation = targetHex.getBaseLevelofHex() + target.getLevelInHex() + leveladj;
        }
        /**
    ...
    protected boolean applyLOSRules(LOSStatus status, LOSResult result) {
        ...
        // need to do source/target separately from intervening hexes
        // check vaslGameInterface is initialized
        if(status.vaslGameInterface != null )  {
           // check tempHex is not source/target and contains a terrain counter
           if(status.tempHex != status.sourceHex && status.tempHex != status.targetHex && status.vaslGameInterface.getTerrain(status.tempHex)!= null) {
              // check for entrenchment counters
             if (!getTerrain(status.vaslGameInterface.getTerrain(status.tempHex)).isEntrenchmentTerrain()) {
        ...
        if (!status.tempHex.equals(status.currentHex)) {
            ...
            if (Math.abs(status.sourceElevation - status.currentHex.getBaseLevelofHex()) != status.rangeToSource) {
            ...
            if(losFollowsDepression(status, sourceadj, targetadj, newequalsprevioushex)) {
        ...
        for (Integer hexside : hexsides) {
            ...
            if (counter != null){
                if (counter.getName().contains("Overlay")) {
                    ...
                    // los is along hexside which has terrain
...
*/
    ...
    // copied from ASLMap - refactor
    private Terrain getOverlayTerrainfromColor(Color color, LOSStatus status){
        int terrint = 0;
        for (Board b: status.vaslGameInterface.gameMap.getBoards()) {
            VASLBoard useboard = (VASLBoard) b;
            if (useboard.getVASLBoardArchive() != null) {
                terrint = useboard.getVASLBoardArchive().getTerrainForColor(color);
                break;
            }
        }
        if (terrint >= 0) {
            return getTerrain(terrint);
        }
        return null;
    }
    ...
    private void applyInterveningCounterValues(LOSStatus status){
        ...
        if (status.vaslGameInterface.getLevel(status.tempHex) != -99) {
            status.groundLevel = status.vaslGameInterface.getLevel(status.tempHex);
        }
    };

    private void applySourceTargetCounterValues(LOSStatus status){
        if (status.vaslGameInterface.getLevel(status.sourceHex) != -99) {
           status.sourceElevation = status.vaslGameInterface.getLevel(status.sourceHex);

        }
        if (status.vaslGameInterface.getLevel(status.targetHex) != -99 ) {
            status.targetElevation = status.vaslGameInterface.getLevel(status.targetHex);

        }
    }

    private GamePiece getOverlayPiece(LOSStatus status){
            ...
            if (status.currentHex.containsExtended(testpoint.x, testpoint.y)) {  // make sure overlay is in current hex
                if (aP instanceof VASSAL.counters.Stack) {
                    for (PieceIterator pi = new PieceIterator(((Stack) aP).getPiecesIterator()); pi.hasMoreElements(); ) {
                        ...
                        if (piece.getProperty("overlay") != null) {  // is overlay
                            return piece;
                            //status.currentTerrain = getTerrainFromOverlayImage(status, piece);
                            //break;
                        }
                    }
                } else {
                    return aP;
                    //status.currentTerrain = getTerrainFromOverlayImage(status, aP);
                    //break;
                }
            }
        }
        ...
        if (status.source.getHex().equals(status.target.getHex())) {
            ...
            if (status.source.isCenterLocation() ||
                ...
                if (status.source.getTerrain().isBuildingTerrain() && status.target.getTerrain().isBuildingTerrain()) {
                    if (Math.abs(status.source.getLevelInHex() - status.target.getLevelInHex()) > 1 ||
    ...
    protected static boolean locationInSmoke(Location location, Smoke smoke) {
        return location.getLevelInHex() >= smoke.getLocation().getLevelInHex() &&
                location.getLevelInHex() < smoke.getLocation().getLevelInHex() + smoke.getHeight();
    }
    ...
    protected boolean checkOBAHindranceRule(LOSStatus status, LOSResult result) {
        ...
        if(status.vaslGameInterface != null) {
            ...
            if (obaList != null && !obaList.isEmpty()) {
                ...
                for (OBA oba: obaList) {
                    // dont add OBA Hindrance if SMOKE
                    if (oba.getName().contains("Sm ") || oba.getName().contains("WP")) {
                        // do nothing
                    }
                    // must be within the blast area
                    else if (oba.getisBarrage()) { //Barrage
                        Hex barrageHex;
                        // test four hexes from center hex on each side depending on rotation
                        String [] barragehexes = getBarrageHexes(oba);
                        for (int x =0; x <9; x++){
                            if (barragehexes[x] != null) {
                                barrageHex = getHex(barragehexes[x]);
                                if (barrageHex.equals(status.currentHex)) {
                                    if (addOBAHindranceAndCheckBlocked(result, oba, status)) {
                                        return true;
                                    }
                                    break;
                                }
                        ...
                        if (range(status.currentHex, oba.getHex(), getMapConfiguration()) <= oba.getBlastAreaRadius()) {
                            ...
                            if (locationInOBA(status.source, oba) || locationInOBA(status.target, oba)) {
                                if (addOBAHindranceAndCheckBlocked(result, oba, status)) {return true;}
                            }
                        ...
                        else if (oba.getName().contains("Naval")){
                            String[] ffehexes = getAllAdjacentHexes(oba.getHex());
                            Hex[] nobahexes = status.vaslGameInterface.getExtraNobaHexes(oba.getRotation(), ffehexes);
                            if (status.currentHex.equals(nobahexes[0]) || status.currentHex.equals(nobahexes[1])){
                                if (addOBAHindranceAndCheckBlocked(result, oba, status)) {return true;}
                            }
                        }
                    }
    ...
    private  boolean addOBAHindranceAndCheckBlocked(LOSResult result, OBA oba, LOSStatus status) {
    ...
    public String[] getBarrageHexes(OBA oba) {
        String[] barragearray = new String[9];
        barragearray[0] = oba.getHex().getName();
        Hex firstarmstartHex = oba.getHex(); Hex secondarmstartHex = oba.getHex();
        int otherhexside = oba.getHex().getOppositeHexside(oba.getRotation()-1);
        int hexside = 0; int secondhexside = 0;
        // ToDo - refactor after beta test
        switch (oba.getRotation()) {
            case 1:
                for (int x = 1; x < 5; x++) {
                    if (getAdjacentHex(firstarmstartHex, oba.getRotation()-1) != null) {
                        barragearray[x] = getAdjacentHex(firstarmstartHex, oba.getRotation() - 1).getName();
                        firstarmstartHex = getAdjacentHex(firstarmstartHex, oba.getRotation() - 1);
                    }
                    if (getAdjacentHex(secondarmstartHex, otherhexside) != null) {
                        barragearray[x+4] = getAdjacentHex(secondarmstartHex, otherhexside).getName();
                        secondarmstartHex = getAdjacentHex(secondarmstartHex, otherhexside);
                    }
                }
                break;
            case 2:
                if (oba.getName().contains("alt")){
                    for (int x = 1; x < 5; x++) {
                        if (x == 1 || x == 3){
                            hexside = 1; secondhexside = 3;
                        } else {
                            hexside = 0; secondhexside = 4;
                        }
                        if (getAdjacentHex(firstarmstartHex, hexside) != null) {
                            barragearray[x] = getAdjacentHex(firstarmstartHex, hexside).getName();
                            firstarmstartHex = getAdjacentHex(firstarmstartHex, hexside);
                        }
                        if (getAdjacentHex(secondarmstartHex, secondhexside) != null) {
                            barragearray[x+4] = getAdjacentHex(secondarmstartHex, secondhexside).getName();
                            secondarmstartHex = getAdjacentHex(secondarmstartHex, secondhexside);
                        }
                    }
                } else {
                    for (int x = 1; x < 5; x++) {
                ...
                break;
            case 3:
                otherhexside = oba.getHex().getOppositeHexside(oba.getRotation()-2);
                for (int x = 1; x < 5; x++) {
                    if (getAdjacentHex(firstarmstartHex, oba.getRotation()-2) != null) {
                        barragearray[x] = getAdjacentHex(firstarmstartHex, oba.getRotation() - 2).getName();
                        firstarmstartHex = getAdjacentHex(firstarmstartHex, oba.getRotation() - 2);
                    }
                    if (getAdjacentHex(secondarmstartHex, otherhexside) != null) {
                        barragearray[x+4] = getAdjacentHex(secondarmstartHex, otherhexside).getName();
                        secondarmstartHex = getAdjacentHex(secondarmstartHex, otherhexside);
            ...
            case 4:
                if (oba.getName().contains("alt")){
                    for (int x = 1; x < 5; x++) {
                        if (x == 1 || x == 3){
                            hexside = 2; secondhexside = 4;
                        } else {
                            hexside = 1; secondhexside = 5;
                        }
                        if (getAdjacentHex(firstarmstartHex, hexside) != null) {
                            barragearray[x] = getAdjacentHex(firstarmstartHex, hexside).getName();
                            firstarmstartHex = getAdjacentHex(firstarmstartHex, hexside);
                ...
                break;
            case 5:
                otherhexside = oba.getHex().getOppositeHexside(oba.getRotation()-3);
                for (int x = 1; x < 5; x++) {
                    if (getAdjacentHex(firstarmstartHex, oba.getRotation()-3) != null) {
                        barragearray[x] = getAdjacentHex(firstarmstartHex, oba.getRotation() - 3).getName();
                        firstarmstartHex = getAdjacentHex(firstarmstartHex, oba.getRotation() - 3);
                    }
                    if (getAdjacentHex(secondarmstartHex,otherhexside) != null) {
                        barragearray[x+4] = getAdjacentHex(secondarmstartHex, otherhexside).getName();
                        secondarmstartHex = getAdjacentHex(secondarmstartHex, otherhexside);
            ...
            case 6:
                if (oba.getName().contains("alt")){
                    for (int x = 1; x < 5; x++) {
                        if (x == 1 || x == 3){
                            hexside = 2; secondhexside = 0;
                        } else {
                            hexside = 3; secondhexside = 5;
                        }
                        if (getAdjacentHex(firstarmstartHex, hexside) != null) {
                            barragearray[x] = getAdjacentHex(firstarmstartHex, hexside).getName();
                            firstarmstartHex = getAdjacentHex(firstarmstartHex, hexside);
    ...
    public String[] getAllAdjacentHexes(Hex passhex) {
    ...
    private boolean locationInOBA(Location location, OBA oba) {
        ...
        return  range(oba.getHex(), location.getHex(), getMapConfiguration()) <= oba.getBlastAreaRadius() &&
                location.getLevelInHex() >= oba.getHex().getCenterLocation().getLevelInHex() &&
                location.getLevelInHex() < oba.getHex().getCenterLocation().getLevelInHex() + oba.getBlastHeight();
    }
    ...
    protected boolean checkDepressionRule(LOSStatus status, LOSResult result) {
        ...
        if (status.exitsSourceDepression) {
            ...
            int currentHexBaseHeight=status.currentHex.getBaseLevelofHex();
            ...
            if (status.exitHexsideIsCrest() && (status.LOSis60Degree || status.LOSisHorizontal) && finalSourceElevation >= finalTargetElevation && !(status.source.getTerrain().isBridge())) {
                status.blocked = true;
        ...
        if (status.entersTargetDepression) {
            ...
            if (status.enterHexsideIsCrest() && (status.LOSis60Degree || status.LOSisHorizontal) && finalTargetElevation >= finalSourceElevation && !(status.target.getTerrain().isBridge())) {
                status.blocked = true;
    ...
    private boolean specialtestDepressionGroundLevelOnEntry (LOSStatus status) {
        ...
        if (status.sourceElevation <= status.currentHex.getBaseLevelofHex()) {return false;}
    ...
    private boolean specialtestDepressionGroundLevelOnExit (LOSStatus status) {
        ...
        if (status.targetElevation <= status.currentHex.getBaseLevelofHex()) {return false;}
        if (status.currentHex.isDepressionTerrain() &&
                status.targetHex.getHexCenter().distance(status.currentCol, status.currentRow)  < status.targetHex.getHexCenter().distance(status.currentHex.getHexCenter().getX(), status.currentHex.getHexCenter().getY())) {
            return true;
        }
    ...
    private boolean losFollowsDepression(LOSStatus status, double sourceadj, double targetadj, boolean newequalsprevioushex) {
        int rangeadj = 0;
        if (status.exitsSourceDepression && !newequalsprevioushex) {  //LOS leaves a depression and we are in a different hex than previously tested
            // LOS cannot follow along a depression if it is along hexside
            if (!(status.LOSisHorizontal) && !(status.LOSis60Degree)) {
                // get the Location closest to the point being tested
                Location testlocation=status.currentHex.getNearestLocation(status.currentCol, status.currentRow);
                // test that target is higher - LOS must leave depression at some point
                if (status.targetElevation + targetadj > status.sourceElevation+ sourceadj) {
                    // get the hexsides crossed by the lost, both entry and exit of current hex
                    HashSet<Integer> testhexsides = status.getHexsideCrossed(status.currentHex);
                    // handle possible error
                    if (testhexsides != null && !testhexsides.isEmpty()) {
                        // for each of the entry/exit hexesides test if matches location nearest to point being tested
                        for (Integer hexside : testhexsides) {
                            if (status.currentHex.getHexsideLocation(hexside).equals(testlocation)) {
                                // if hexside being crossed is depression/water need to test further
                                if (status.currentHex.getHexsideLocation(hexside).isDepressionTerrain() | status.currentHex.getHexsideLocation(hexside).getTerrain().isWaterTerrain()) {
                                    ...
                                    return (!((status.targetElevation +targetadj) - (status.sourceElevation+ sourceadj) >= status.rangeToSource - rangeadj) ? false : true);
                                }
                            }
                        }
                    }
        ...
        if (status.entersTargetDepression && !newequalsprevioushex) { //LOS enters a depression and we are in a different hex than previously tested
            // LOS cannot follow along a depression if it is along hexside
            if (!(status.LOSisHorizontal) && !(status.LOSis60Degree)) {
                // get the Location closest to the point being tested
                Location testlocation=status.currentHex.getNearestLocation(status.currentCol, status.currentRow);
                // test that source is higher - LOS must enter depression at some point
                    ...
                    if (testhexsides != null && !testhexsides.isEmpty()) {
                        ...
                        for (Integer hexside : testhexsides) {
                            if (!status.currentHex.getHexsideLocation(hexside).equals(testlocation)) {
                                ...
                                if (status.currentHex.getHexsideLocation(hexside).isDepressionTerrain() | status.currentHex.getHexsideLocation(hexside).getTerrain().isWaterTerrain()) {
                                    rangeadj = status.rangeToSource - 1;
                                    return (!((status.sourceElevation + sourceadj) - (status.targetElevation + targetadj) >= status.rangeToSource - rangeadj) ? false : true);
                                }
                            }
                        }
                    }
            ...
            // code added by DR to handle cellars
                ...
                if (!ignore) {
                    ...
                    if (BOCAGE.equals(status.currentTerrain.getName())) {
                        ...
                        //same height as source/target, but other is higher; or lower than both - no Block or Hind
                        ...
                        //higher than both source/target - Hind
                        ...
                        //same height as higher source/target, but other is lower
                        else //if ((status.groundLevel + status.currentTerrainHgt == Math.max(status.sourceElevation, status.targetElevation) &&
                            //status.groundLevel + status.currentTerrainHgt > Math.min(status.sourceElevation, status.targetElevation)) && !status.slopes)
                        {
                            // otherwise check for blind hexes
                            if (isBlindHex(status, status.currentTerrainHgt)) {
                                status.reason = "Source or Target location is in a blind hex (B14.2)";
                                status.blocked = true;
                                result.setBlocked(status.currentCol, status.currentRow, status.reason);
                                return true;
                            }
            ...
            if(status.LOSis60Degree || status.LOSisHorizontal) {  //need hexside to be cliff
                if (status.rangeToSource % 2 != 0) { // at odd range check for cliff
                    ...
                    if(x !=-1 && status.currentHex.getHexsideLocation(x).getTerrain().isCliff()) {
                        ...
                        if(!temp.equals(status.sourceHex) && !temp.equals(status.targetHex)){
                            cliffHexsideTerrainHeightadjustment= Math.min(status.currentHex.getBaseLevelofHex(), temp.getBaseLevelofHex());
                        }
                    }
                }
            }
    ...
    protected boolean  checkTerrainHeightRule(LOSStatus status, LOSResult result) {
        ...
        int rooftopadj=0;
        Hex rangehex=null;
        if(status.source.getTerrain().isRooftop() && status.source.getLevelInHex() !=1) {  // hack to handle SK (all buildings single story
            sourceadj=-0.5;
            rooftopadj=1;
            rangehex=status.sourceHex;
        }
        if(status.target.getTerrain().isRooftop() && status.target.getLevelInHex() !=1) {  // hack to handle SK (all buildings single story) {
            targetadj=-0.5;
            rooftopadj=1;
            rangehex=status.targetHex;
        }
        ...
        if ( (status.groundLevel + status.currentTerrainHgt + obstacleadj== Math.max(status.sourceElevation + sourceadj, status.targetElevation + targetadj)) &&
            ...
            if (status.rangeToSource==1 && (status.currentTerrainHgt+ obstacleadj==0 && status.sourceElevation> status.currentHex.getBaseLevelofHex()) &&
                    status.sourceHex.getHexCenter().distance(status.currentCol, status.currentRow)  < status.sourceHex.getHexCenter().distance(status.currentHex.getHexCenter().getX(), status.currentHex.getHexCenter().getY())) {
                return false;
            }
            if (status.rangeToTarget==1 && (status.currentTerrainHgt+ obstacleadj==0 && status.targetElevation> status.currentHex.getBaseLevelofHex()) &&
                    status.targetHex.getHexCenter().distance(status.currentCol, status.currentRow)  < status.targetHex.getHexCenter().distance(status.currentHex.getHexCenter().getX(), status.currentHex.getHexCenter().getY())) {
                return false;
            }
            ...
            if(status.LOSis60Degree || status.LOSisHorizontal) {  //need hexside to be cliff
                if (status.rangeToSource % 2 != 0) { // at odd range check for cliff
                    ...
                    if(x !=-1 && status.currentHex.getHexsideLocation(x).getTerrain().isCliff()) {
                        ...
                        // ignore any source/target hex
                        if(!temp.equals(status.sourceHex) && !temp.equals(status.targetHex)){
                            lowerHexheight= Math.min(status.currentHex.getBaseLevelofHex(), temp.getBaseLevelofHex());
                        } else {
                            lowerHexheight = status.currentHex.getBaseLevelofHex();
                        }
 ...
 */
    ...
    protected boolean checkHalfLevelTerrainRule(LOSStatus status, LOSResult result) {
        ...
        double sourceadj=0;
        double targetadj=0;
        if(status.source.getTerrain().isRooftop() && status.source.getLevelInHex() !=1 && !(status.target.getTerrain().isRooftop())) {
            sourceadj=-0.5;
        }
    ...
    public boolean isIgnorableHexsideTerrain(Hex h, Location l, int LOSHexspine, LOSStatus status) {
        ...
        if (status.vaslGameInterface != null && status.vaslGameInterface.getHexside(status.currentHex) !=null){
            ...
            if (counter.getTerrain().equals(status.currentTerrain.getName())){
            ...
            } else if (locationHexside == counter.getCoverArch()) {
        ...
        if (isHexspine(h, l) && (locationHexsideTerrain != null)) {
            if (BOCAGE.equals(locationHexsideTerrain.getName()) || PARTIALORCHARD.equals(locationHexsideTerrain.getName())) {
                return false;
            } else {
                return true;
            }
    ...
    protected static boolean isBlindHex(LOSStatus status, int terrainHeight, boolean isCliffHexside, int cliffHexsideTerrainHeightadjustment) {
        ...
        // los crossing cliff hexside not along hexside
     ...
     * if number of columns is odd then hex grid will be symetrical
     * on symetrical map left and right side columns will have the same height configuration - whether top hex in column is full height or half height
     * adjust hex grid by changing value of A1CenterY
     * Width
     * if both left and right edges have same hex width (full hex or half hex) no need to adjust grid
    ...
    public boolean insertMap(Map map, Hex upperLeft) {
        ...
        if((upperLeft.getColumnNumber()>0 || upperLeft.getRowNumber() >0)) {   // inserting a board in second or greater column
            if(this.cropconfiguration.contains("Normal") && upperLeft.getColumnNumber() % 2 == 0 && !(map.cropconfiguration.contains("Normal") || map.cropconfiguration.contains("Topleftfullheight"))) {
                // previous board has full-height "Normal" config in final col; new board must match
                return false;
            }
            else if (this.cropconfiguration.contains("Normal") && upperLeft.getColumnNumber() % 2 != 0 && !map.cropconfiguration.contains("TopleftHalfHeight"))  {
                // previous board has half-height "TopleftHalfHeight" config in final col; new board must match
                return false;
            }
            else if (this.cropconfiguration.contains("TopleftHalfHeight") && upperLeft.getColumnNumber() % 2 ==0  && !map.cropconfiguration.contains("TopleftHalfHeight")) {
                // previous board has half-height "TopleftHalfHeight" config in final col; new board must match
                return false;
            }
            else if (this.cropconfiguration.contains("TopleftHalfHeight") && upperLeft.getColumnNumber() % 2 !=0  && !map.cropconfiguration.contains("Normal")) {
                // previous board has full-height "Normal" config in final col; new board must match
                return false;
            }
        }
        ...
        if(left==0 && upper==0) {
            ...
            if (this.cropconfiguration.contains("HalfHeight") || map.flipconfiguration.contains("HalfHeight")) {
                ...
                this.flipconfiguration = map.flipconfiguration;
            }
            if (map.flipconfiguration =="FulltoHalfWidth") {  // need to handle halftofull and odd even col
                //need to redo the VASLMap hexgrid
                A1CenterX=0;
                int evencol = 0;
                for (int col = 0; col < this.width; col++) {
                ...
                this.flipconfiguration = map.flipconfiguration;
            } else if(map.flipconfiguration =="HalftoFullWidth") {
                //need to redo the VASLMap hexgrid
                ...
                this.flipconfiguration = map.flipconfiguration;
            } else if(map.flipconfiguration =="HalftoFullHeight") {
                //need to redo the VASLMap hexgrid
                A1CenterX = this.cropconfiguration.contains("FullHex") ? this.hexWidth/2 : 0;
                int evencol = 0;
                for (int col = 0; col < this.width; col++) {
                    evencol = col % 2 == 0 ? 0 : 1;
                    hexGrid[col] = new Hex[this.height + evencol]; // add 1 if odd
     ...
     */
    public Map  crop(Point upperLeft, Point lowerRight, int offsetsize, double passA1centerx, double passA1centery){ // passboardgridconfig, String passcropgridconfig){
        ...
        int offadj = 0; int fullhexadj=0;
        // the next line is necessary hack when cropping to full hex as crop bound (which is used for upperLeft.x) is right on hex border and produces inconsistent results so push to hex center
        if (passcropgridconfig.contains("FullHex") && !(passcropgridconfig.contains("LeftHalf"))) {fullhexadj= (int) hexWidth/2;}
        ...
        if(passcropgridconfig.contains("FullHex")) {
            //upperLeftHex = gridToHex(upperLeft.x + fullhexadj + offadj, upperLeft.y);
            ...
            if (passcropgridconfig.contains("LeftHalf")) {A1CenterX = 0;}
        ...
        // determine if upperLeftHex is half height or full height
        String uplheight = getheightupperLeftHex(upperLeftHex, passboardgridconfig);
        passcropgridconfig = passcropgridconfig + uplheight;
        /*if (!(upperLeftHex.getColumnNumber() %2 ==0) && A1CenterY !=65) {
            passcropgridconfig= passcropgridconfig + "TopleftHalfHeight";
            A1CenterY = 0;
        }*/

        /*if (passcropgridconfig.contains("ROadjustment")) {
            passcropgridconfig += "ROadjustment";
        }*/

        // set cropconfiguration to handle special cases (offset A row (ie RB, RO, DaE) : equal rows in every col (ie DaE))
        if(this.cropconfiguration.contains("EqualRowCount") && !passcropgridconfig.contains("EqualRowCount") ) {passcropgridconfig = passcropgridconfig + "EqualRowCount";}
        if(this.cropconfiguration.contains("Offset")) { passcropgridconfig=passcropgridconfig + "Offset";}
        ...
        this.cropconfiguration =passcropgridconfig;
    ...
    private String getheightupperLeftHex(Hex upperLeftHex, String passboardgridconfig){
        ...
        int oddevenrow = upperLeftHex.getRowNumber() % 2 == 0 ? 0 : 1;
        if (passboardgridconfig.contains("TopleftHalfHeight")) {  //RO is currently only known board
            switch (oddevencol) {
                case 0: // even col - first col is "0" - will always be full height unless first row
                    switch (upperLeftHex.getRowNumber()) {
                        case 0: // half height
                            returnstring = "TopleftHalfHeight";
                            break;
                        default:
                            returnstring = "Topleftfullheight";
                            break;
                    }
                    break;
                case 1:
                    switch (oddevenrow) {  // result will vary depending on row value
                        case 0:
                            returnstring = "TopleftHalfHeight";
                            break;
                        case 1:
    ...
    public boolean checkRBrrembankments (LOSStatus status,LOSResult result, HashSet<Integer> hexsides) {
        ...
        int hexside=status.currentHex.getLocationHexside(status.currentHex.getNearestLocation(status.currentCol, status.currentRow));
        if (hexside!=-1) {
            boolean[] rrembanks = new boolean[6];
            rrembanks = status.currentHex.getRBrrembankments();
            if (rrembanks[hexside]) {
                checkhexside = status.currentHex.getHexsideTerrain(hexside);
                if (checkhexside != null) {
            ...
            if (partialorchards[hexside]) {
                ...
                if (checkhexside != null) {
                    ...
                    if (checkhexside.isHexsideTerrain() && checkhexside.getName().contains(PARTIALORCHARD)) {
                        ...
                        int heightadj = 0;
                        if (status.currentTerrain.getName().contains("Water")) {
                            status.groundLevel += 1;
                        }
                        status.currentTerrain = getTerrain(PARTIALORCHARD);
                        //status.currentTerrain.setName("");
                        status.currentTerrainHgt = status.currentTerrain.getHeight();
    ...
    public boolean checkRowhouseFactoryWallAndBreach(LOSStatus status, LOSResult result) {
        ...
        if (status.currentTerrain.getName().contains("Interior Factory Wall") && (result.isLOSis60Degree() | result.isLOSisHorizontal()) ) {
            ...
            //same height as source/target, but other is higher
            else if((status.groundLevel + status.currentTerrainHgt < Math.max(status.sourceElevation + sourceadj, status.targetElevation + targetadj) &&
                    status.groundLevel + status.currentTerrainHgt == Math.min(status.sourceElevation + sourceadj, status.targetElevation + targetadj))
            ){
                return false;
            }
    ...
    public String getMapConfiguration() {
        //this works for one board geo maps only
        return this.cropconfiguration;

        //ToDo fix for all maptypes
        ...
        if (this.flipconfiguration.equals("FulltoHalfHeight")){
            return "TopleftHalfHeight";
        }
        return "Normal";*/
    }
