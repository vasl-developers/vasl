...
NOTE: this code will now work with geo maps – both the 33 x 10 hex and 17 x 20 hex sizes and those
HASL maps which previously supported LOS.

None of the changes have caused games to crash during testing to date. All that will happen is that the
LOS Engine will fail when a game is opened if a bug is encountered. It will be safe to continue play but
only the basic VASSAL LOS string will display.
